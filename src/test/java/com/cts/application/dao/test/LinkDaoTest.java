...
public class LinkDaoTest extends TestCase {
	...
	public void testGetLinks() {
		...
		k = new BeanPropertyRowMapper<Link>(Link.class);
		// Mockito.when(new
		// BeanPropertyRowMapper<Link>(Link.class)).thenReturn(k);
		Mockito.when(this.jdbc.query(sqla, k)).thenReturn(listOfLinks);
		assertEquals(listOfLinks.toString(), linkDao.getLinks(linkname).toString());
	...
	public void testGetSubLinks() {
		...
		// BeanPropertyRowMapper<Page>
		// k=Mockito.any(BeanPropertyRowMapper.class);
